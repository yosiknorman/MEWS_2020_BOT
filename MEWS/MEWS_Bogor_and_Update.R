...
EXIST_Penurunan = function(PARA){
  ...
  res = res[!is.na(res)]
  res = (length(which(res < 0))/length(res))*100 # Berapa persen jumlah grid yang aktivitas konvektifnya meningkat
  return(res)
}
...
if(TEXT != "No Warning"){
  ...
  if(Interval_Waktu > 0){
    if(any(pertambahan > 0.25)){
      ...
      print(TEXTUPD)
      write.table(file = paste0("Verifikasi/tiap_warning/", WAKTU_REL, ".txt"), ARSIP_Verifikasi, row.names = F)
    }else{
